...
static void clear_in_events(Statechart* handle)
{
	handle->iface.GetSample_raised = bool_false;
}
...
void statechart_raise_getSample(Statechart* handle)
{
	statechart_add_event_to_queue(&(handle->in_event_queue), Statechart_GetSample);
	run_cycle(handle);
}
...
static sc_integer main_region_WaitSamplePeriod_react(Statechart* handle, const sc_integer transitioned_before)
{
	...
	{ 
		...
		{ 
			if (handle->iface.GetSample_raised == bool_true)
			{ 
				exseq_main_region_WaitSamplePeriod(handle);
...
static sc_boolean statechart_dispatch_event(Statechart* handle, const statechart_event * event) {
	switch(event->name) {
		case Statechart_GetSample:
		{
			handle->iface.GetSample_raised = bool_true;
			return bool_true;
		}
		default:
			return bool_false;
